...
def extract_text_segments_from_alignment(utt_obj: Dict, alignment: List[int], output_timestep_duration: float) -> List[
    ...
    # Get segments from utt_obj
    if 'segment_utt_map' in utt_obj:
        for segment_data in utt_obj['segments_utt_info']:
            segment_id = segment_data['segment_id']
            segment_text = segment_data.get('text', '')

            # Find alignment indices for this segment
            segment_start_idx = segment_data.get('segment_token_span')[0]
            segment_end_idx = segment_data.get('segment_token_span')[1]

            # Get token alignments for this segment
            token_alignments = []
            for token_idx in range(segment_start_idx, segment_end_idx):
                if token_idx < len(utt_obj['token_level_details']):
                    ...
                    if 'aligned_w_token' in token_data:
                        token_alignments.append(token_data['aligned_w_token'])

            if token_alignments:
                # Calculate segment timestamps
                start_time = min(a[1] for a in token_alignments) * output_timestep_duration
                end_time = max(a[2] for a in token_alignments) * output_timestep_duration
    ...
    # Fallback: use word-level alignments if segments not available
    elif 'words_info' in utt_obj:
        for word_data in utt_obj['words_info']:
            if 'aligned_w_word' in word_data:
                word_alignment = word_data['aligned_w_word']
                start_time = word_alignment[1] * output_timestep_duration
                end_time = word_alignment[2] * output_timestep_duration
